...
ident-alt.yang:  This is an alternative, that doesn't work unless ieee802-dot1q-bridge.yang changes.  There is a new identity created called ieee-link-agg and a new identity called ieee8021axLag.  The identity ieee-link-agg has both ieee8021axLag and ieee8023adLag as bases.  This fails because the new ieee8021axLag identity is not based on the ieee8023adLag identity, so the when statement returns false.  This would work if the ieee802-dot1q-bridge.yang module could change to use the ieee-link-agg identity in the when statement, and a modification is made to the IANA registry to add a base to ieee8023adLag (see ident-mod example).
...
ident-mod.yang:  This is another positive example.  This modifies the when statement in ident-alt.yang to show this is viable if ieee802-dot1q-bridge.yang and the IANA iana-if-type.yang module is modified.
